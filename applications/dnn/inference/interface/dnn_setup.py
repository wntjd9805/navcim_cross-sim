...
def build_keras_model(model_name,show_model_summary=False):
    ...
    if model_name == "Resnet50":
        ...
        from helpers.keras_model_builders.buildMobilenetv2_int8 import MobileNetV2_int8
        weights_file = model_dir + "imagenet/MLperf/mobilenetv2_int8-weights.p"
        with open(weights_file, 'rb') as f:
            weights_dict = pickle.load(f)
        # weights_dict = pickle.load(open(weights_file,"rb"))
        weight_limits_file = model_dir + "imagenet/MLperf/mobilenetv2_int8-weight_limits.p"
        weight_limits_dict = pickle.load(open(weight_limits_file,"rb"))
        keras_model = MobileNetV2_int8(weights_dict,weight_limits_dict)
...
'''
def load_adc_activation_ranges(config,positiveInputsOnly,NrowsMax_ = None, NcolsMax_ = None):
    ...
    limits_dir = f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/"
    ...
    if adc_bits > 0 and adc_range_option == "CALIBRATED":
        ...
        if model_name == "Resnet50":
            adc_ranges = np.load(limits_dir+"examples/adc_limits_Resnet50_"+str(NrowsMax)+"rows_"+str(NcolsMax)+"cols_"+str(Nslices)+"slices_BALANCED.npy")
            # if Nslices == 1:
        ...
        # CIFAR10 ResNet models
            ...
            if Nslices == 1 and style == "BALANCED" and NrowsMax >= 1152 and not ADC_per_ibit and subtract_current_in_xbar:
               adc_ranges = np.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/imagenet/adc_limits_VGG19_balanced1152.npy")
        
        elif model_name == "Resnet18":
            adc_ranges = np.load(limits_dir+"examples/adc_limits_Resnet18_"+str(NrowsMax)+"rows_"+str(NcolsMax)+"cols_"+str(Nslices)+"slices_BALANCED_reluAware_candidate.npy")
            ...
            if Nslices == 1:
                adc_ranges = np.load(limits_dir+"examples/adc_limits_MobilenetV2_"+str(NrowsMax)+"rows_"+str(NcolsMax)+"cols_"+str(Nslices)+"slices_BALANCED_reluAware_candidate.npy")
            else:
                adc_ranges = np.load(limits_dir+"examples/adc_limits_MobilenetV2_"+str(NrowsMax)+"rows_"+str(NcolsMax)+"cols_"+str(Nslices)+"slices_BALANCED.npy")
        elif model_name == "VGG11" and Nslices == 1:
            adc_ranges = np.load(limits_dir+"examples/adc_limits_VGG11_"+str(NrowsMax)+"rows_"+str(NcolsMax)+"cols_"+str(Nslices)+"slices_BALANCED_reluAware_candidate.npy")
        elif model_name == "VGG16" and Nslices == 1:
            adc_ranges = np.load(limits_dir+"examples/adc_limits_VGG16_"+str(NrowsMax)+"rows_"+str(Nslices)+"slices_BALANCED_reluAware_candidate.npy")
        elif model_name == "SqueezeNet":
            if Nslices == 1:
                adc_ranges = np.load(limits_dir+"examples/adc_limits_SqueezeNet_"+str(NrowsMax)+"rows_"+str(NcolsMax)+"cols_"+str(Nslices)+"slices_BALANCED_reluAware_candidate.npy")
            else:
                adc_ranges = np.load(limits_dir+"examples/adc_limits_SqueezeNet_"+str(NrowsMax)+"rows_"+str(NcolsMax)+"cols_"+str(Nslices)+"slices_BALANCED.npy")
    ...
    if dac_bits > 0 or model_name == "Resnet50-int4":
        if model_name == "Resnet50-v1.5":
            ...
            dac_ranges = np.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/examples/dac_limits_Resnet50.npy")
        elif model_name == "Resnet50-int4":
            dac_ranges = np.load(limits_dir+"imagenet/dac_limits_ResNet50int4.npy")
        elif model_name == "VGG19":
            dac_ranges = np.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/examples/dac_limits_VGG19.npy")
            # dac_ranges = [np.array([0,6])] * Nlayers_mvm
        elif model_name == "ResNet56":
            ...
            dac_ranges = np.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/examples/dac_limits_mobilenet_v2.npy")
        elif model_name == "VGG16":
            dac_ranges = np.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/examples/dac_limits_VGG16.npy")
        elif model_name == "VGG11":
            dac_ranges = np.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/examples/dac_limits_VGG11.npy")
        elif model_name == "Resnet18":
            dac_ranges = np.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/examples/dac_limits_ResNet18.npy")
        elif model_name == "SqueezeNet":
            dac_ranges = np.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/adc/adc_limits/examples/dac_limits_SqueezeNet.npy")
        else:
            raise ValueError("DAC limits not available for the chosen model")
        ...
        for idx, dac_range in enumerate(dac_ranges):
            if dac_range[0] < 0:
                positiveInputsOnly[idx] = False

    return adc_ranges, dac_ranges, positiveInputsOnly
