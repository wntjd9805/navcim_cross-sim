...
parser.add_argument('--NcolsMax', type=int, default=0, help='NrowsMax')
...
if config.model_name== 'VGG19':
    onnx_model = onnx.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/vgg19.onnx")
elif config.model_name== 'VGG16':
    onnx_model = onnx.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/vgg16.onnx")
elif config.model_name== 'Resnet18':
    onnx_model = onnx.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/resnet18.onnx")
    quantize =  False
elif config.model_name== 'Resnet50':
    onnx_model = onnx.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/resnet50_int8.onnx")
    quantize =  True
elif config.model_name== 'MobileNetV2':
    onnx_model = onnx.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/mobilenetv2_int8.onnx")
    quantize =  True
elif config.model_name== 'Mnasnet1_0':
    onnx_model = onnx.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/mnasnet1_0.onnx")
elif config.model_name== 'Efficientnet_V2_s':
    onnx_model = onnx.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/efficientnet_v2_s.onnx")
elif config.model_name== 'Squeezenet1_0':
    onnx_model = onnx.load(f"{navcim_dir}/cross-sim/applications/dnn/inference/squeezenet1_0.onnx")
...
def calculate_fitness_with_simulation(individual,adc_idx):
...
calculate_fitness_with_simulation(init_idx,init_adc_idx)
