...
output_dir = f"{navcim_dir}/cross-sim/applications/dnn/data/datasets/imagenet/"
# Whether nor not preprocess images in the MLPerf calibration set
MLperf_calibration = True
...
if not MLperf_calibration:
    if num_images == 25000:
        if n_start == 0:
            np.save(output_dir+"x_val_MLperfMobileNet_"+str(num_images)+"_front.npy", x_val)
        elif n_start == 25000:
            np.save(output_dir+"x_val_MLperfMobileNet_"+str(num_images)+"_back.npy", x_val)
    else:
        np.save(output_dir+"x_val_MLperfMobileNet_"+str(num_images)+".npy", x_val)
else:
    np.save(output_dir+"x_val_MLperfMobileNet_cal.npy", x_val)