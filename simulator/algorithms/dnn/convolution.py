...
class Convolution:
    ...
    #     stride = self.stride
    #     NrowsX = Kx * Ky * Nic  # number of rows per sliding window MVM
    #     core_weights = self.core.get_matrix()
    #     # Apply zero padding
    #     M_padded = np.pad(
    #         M_input,
    #         ((0, 0), (self.px_0, self.px_1), (self.py_0, self.py_1)),
    ...
    #     Nx_out = (M_padded.shape[1] - Kx) // stride + 1
    #     Ny_out = (M_padded.shape[2] - Ky) // stride + 1

    #     # Initialize the matrix to store the sliding windows
    #     M_windows = np.zeros((NrowsX, Nx_out * Ny_out))

    #     # Extract each sliding window
    #     idx = 0
    #     for y in range(0, M_padded.shape[1] - Kx + 1, stride):
    #         for x in range(0, M_padded.shape[2] - Ky + 1, stride):
    #             window = M_padded[:, y:y+Kx, x:x+Ky].reshape(-1)
    #             M_windows[:, idx] = window
    #             idx += 1

    #     # Perform matrix multiplication with the conv core's weights
    #     # Assuming 'self.core_weights' is a matrix of size (Noc, NrowsX)
    #     print(core_weights)
    

    #     M_output = np.dot(core_weights, M_windows)
    #     # Reshape the output matrix to the desired 3D shape
    #     M_output = M_output.reshape((Noc, Nx_out, Ny_out))

    #     M_out = self.core.matmat(M_windows)
    #     M_out = M_out.reshape((Noc, Nx_out, Ny_out))
    #     print(M_output)
    #     print(M_out)
